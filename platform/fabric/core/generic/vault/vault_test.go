...
func TestMerge(t *testing.T) {
	...
	// create DB and kvs
	ddb, err := db.OpenVersioned(nil, "memory", "")
...
func TestVaultInMem(t *testing.T) {
	db1, err := db.OpenVersioned(nil, "memory", "")
	assert.NoError(t, err)
	db2, err := db.OpenVersioned(nil, "memory", "")
	assert.NoError(t, err)
	testRun(t, db1, db2)
}

func TestVaultBadger(t *testing.T) {
	db1, err := db.OpenVersioned(nil, "badger", filepath.Join(tempDir, "DB-TestVaultBadgerDB1"))
	assert.NoError(t, err)
	db2, err := db.OpenVersioned(nil, "badger", filepath.Join(tempDir, "DB-TestVaultBadgerDB2"))
	assert.NoError(t, err)
	defer db1.Close()
	defer db2.Close()
